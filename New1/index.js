$(function(){
 ...
 // for showimg create Account and log in container
 $('.bi-person').mouseenter(function(){
     $('#hover-nav-element').stop().empty();
     $('#hover-nav-element').stop()
                             .prepend("<h4>Account</h4>")
                             .append("<button type='button'>Log in</button>")
                             .append("<button type='button'>Create Account</button>")
                             .css({'display':'block','top':$(this)[0].offsetHeight,"left":$(this)[0].offsetLeft-300,'height':'auto','width':'25%','text-align':'center','z-index':1000000});
         $('#hover-nav-element').stop().find('button')
                                         .css({'background-color':'#ADD8E6','width':'90%','margin':'5px','border':'0px','padding':'5px','font-weight':"bold"})
 })
 $('.bi-person').mouseleave(function(){
     $('#hover-nav-element').stop().hide();
 });
